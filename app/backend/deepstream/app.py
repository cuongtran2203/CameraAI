...
def get_roi():
    """Returns a list of ROI dicts. Supports both single-object (backward compat)
    and array format: {"roi": [{"name": "...", "left": ..., ...}, ...]}"""
    ...
    try:
        ...
        if mtime != last_roi_mtime:
            ...
            if isinstance(raw, dict):
                ...
                current_roi = raw if isinstance(raw, list) else []
            last_roi_mtime = mtime
    except Exception:
        pass
    return current_roi  # always a list (possibly empty)
...
def osd_sink_pad_buffer_probe(pad, info, user_data):
    ...
    while l_frame:
        ...
        # --- Draw all ROI rectangles (one display_meta per ROI to avoid
        #     fixed-array bounds issues — works for any number of ROIs) ---
        for roi in rois:
            display_meta = pyds.nvds_acquire_display_meta_from_pool(batch_meta)
        ...
        while l_obj:
            ...
            if obj_meta.class_id == 99:
                ...
                det_left = rect.left
                det_top = rect.top
                det_right = det_left + rect.width
                det_bottom = det_top + rect.height
                ...
                for roi in rois:
                    ...
                    overlap_left = max(det_left, roi_left)
                    overlap_top = max(det_top, roi_top)
                    overlap_right = min(det_right, roi_right)
                    overlap_bottom = min(det_bottom, roi_bottom)
